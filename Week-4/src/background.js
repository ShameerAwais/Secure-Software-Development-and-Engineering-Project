...
chrome.webNavigation.onCompleted.addListener(async (details) => {
    ...
    try {
        ...
        if (isPhishing) {
            // Securely store the detected phishing URL
            await chrome.storage.local.set({ 
                lastPhishingUrl: await secureApi.encryptLogEntry({ url, timestamp: Date.now() })
            });
...
// Ensure user consent and settings are checked before making phishing requests
chrome.storage.sync.get(['enableDetection', 'userConsent'], (settings) => {
  if (!settings.enableDetection || !settings.userConsent) {
    return;
  }
